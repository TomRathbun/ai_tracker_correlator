...
graph TD
    ...
    subgraph "5. Decoding & Extraction"
        ...
        Exit[Existence Logit / Probability]
    end
...
- **7**: Each node is normalized into a 7-element feature vector:
    - **[x, y, z]**: 3D Position.
...
- Pairwise Classifier Score $[prob]$: A pre-calculated probability (from a separate classifier) that these two nodes are the same aircraft. This acts as a "hint" to the attention mechanism.
...
| 6 | prob | 0.963 | Expert Score: Pairwise probability hit belongs to this track. |

3. How the "Intelligence" emerges

When the GNN processes this:

- For Track 0: It looks at the vectors for edges (0,3) and (0,4). Because the prob is high, the Attention mechanism will give these measurements a high weight. The GNN "pulls" their coordinates in to refine Node 0's state.
...
Head 3 (Trust): Looks at the Pairwise Classifier Score. It acts as a "Gatekeeper"—if the expert classifier says the match is garbage, this head assigns zero attention to that edge.
Head 4 (Noise Filter): Looks at the Amplitude and Sensor ID. It learns to ignore "Sensor Jitter" and ensures the fused position doesn't jump around due to one noisy radar reporting a bad coordinate.
The Merger
...
Goal: It maps the abstract "consensus" of the aircraft's behavior into 8 specific values.  

2. The 8 Output Tensors
The 8 numbers produced for every node are split into two categories:
    ...
    - Index 6 (Initiation): "I think this is a new flight entering the sector."
    - Index 7 (Survival): "I think this track is still real and hasn't crashed or flown out of range."

3. The Activation Function (Sigmoid)
The final 2 "Logits" (the existence scores) are passed through a Sigmoid function, which scales them into a probability between 0 and 1.
